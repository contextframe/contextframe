...
def format_as_text(record: FrameRecord) -> str:
    ...
    if 'source' in record.metadata:
        ...
        lines.append(f"Source: {source.get('type', 'unknown')} - {source.get('path', 'N/A')}")
    ...
    if relationships:
        ...
        for rel in relationships:
            lines.append(f"  - {rel.get('relationship_type')} -> {rel.get('target_type')}: {rel.get('target_identifier')}")
...
def format_as_markdown(record: FrameRecord) -> str:
    ...
    if record.raw_data or record.vector is not None:
        ...
        if record.raw_data:
            lines.append(f"- **Raw Data**: {len(record.raw_data)} bytes ({record.raw_data_type or 'unknown type'})")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Get a specific document from a ContextFrame dataset')
