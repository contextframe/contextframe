...
def read_file_content(file_path: Path) -> tuple[str, Optional[bytes], Optional[str]]:
...
def create_record_metadata(file_path: Path, record_type: str, identifier: Optional[str], 
                          collection: Optional[str], chunk_info: Optional[dict] = None) -> dict:
...
def add_file(dataset: FrameDataset, file_path: Path, args) -> List[str]:
        ...
        if len(chunks) > 1:
            ...
            for i, chunk in enumerate(chunks):
                ...
                record = FrameRecord(
                    text_content=chunk,
                    metadata=metadata,
                    raw_data=raw_data if i == 0 else None,  # Only store raw data with first chunk
    ...
    metadata = create_record_metadata(file_path, args.type, args.identifier, args.collection)
    ...
    if args.embeddings:
        ...
        try:
            ...
            records_with_embeddings = create_frame_records_with_embeddings(
                documents=[{"content": text_content, "metadata": metadata}],
                model=model
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Add documents to a ContextFrame dataset')
