...
def search_hybrid(
    dataset: FrameDataset, query: str, limit: int, filter_expr: str | None = None
) -> list:
...
def search_vector(
    ...
    try:
        ...
        print(
            "Make sure you have set up API credentials for your embedding provider.",
            file=sys.stderr,
        )
...
def format_result(record, index: int):
    ...
    for rel in relationships:
        if (
            rel.get('relationship_type') == 'member_of'
            and rel.get('target_type') == 'collection'
        ):
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Search documents in a ContextFrame dataset'
    )
