...
def format_as_table(records: list) -> str:
    ...
    for record in records:
        ...
        updated = record.metadata.get("updated_at", record.metadata.get("created_at", ""))
        if updated:
            try:
...
def format_as_json(records: list) -> str:
    ...
    for record in records:
        data.append({
            ...
            "record_type": record.metadata.get("record_type", "document"),
            "title": record.metadata.get("title"),
            "content_preview": record.text_content[:200] + "..." if len(record.text_content) > 200 else record.text_content,
...
def main():
    ...
    parser = argparse.ArgumentParser(description='List documents in a ContextFrame dataset')
    parser.add_argument('dataset', help='Path to the dataset')
    parser.add_argument('--limit', type=int, default=50, help='Number of records to return')
