...
class CollectionTools:
    ...
    def register_tools(self, tool_registry):
        ...
        for name, handler, schema in tools:
            tool_registry.register_tool(
                ...
                or f"Collection {name.split('_')[1]} operation",
            )
    ...
    async def create_collection(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        header_metadata = {"record_type": "collection_header", "title": validated.name}
            ...
            for doc_id in validated.initial_members:
                try:
                    self._add_document_to_collection(
                        doc_id, collection_id, header_record.metadata.get("uuid")
                    )
                    added_members += 1
                except Exception as e:
                    logger.warning(
                        f"Failed to add document {doc_id} to collection: {e}"
                    )
    ...
    async def update_collection(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        if validated.remove_members:
            for doc_id in validated.remove_members:
                try:
                    self._remove_document_from_collection(
                        doc_id, validated.collection_id
                    )
                    removed_count += 1
                except Exception as e:
        ...
        if validated.add_members:
            for doc_id in validated.add_members:
                try:
                    self._add_document_to_collection(
                        doc_id, validated.collection_id, header.metadata.get("uuid")
                    )
                    added_count += 1
                except Exception as e:
    ...
    async def delete_collection(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        if validated.delete_members:
            ...
            for member in members:
                try:
                    self._remove_document_from_collection(
                        member["uuid"], validated.collection_id
                    )
                except Exception as e:
    ...
    async def list_collections(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        columns = [
            col
            for col in self.dataset._dataset.schema.names
            if col not in ["raw_data", "raw_data_type"]
        ]
        ...
        for batch in scanner.to_batches():
            for i in range(len(batch)):
                ...
                coll_info = CollectionInfo(
                    ...
                    parent_id=record.metadata.get("collection_id")
                    if record.metadata.get("collection_id_type") == "uuid"
                    else None,
                    created_at=coll_meta["created_at"]
                    or record.metadata.get("created_at", ""),
                    updated_at=coll_meta["updated_at"]
                    or record.metadata.get("updated_at", ""),
                    metadata=coll_meta["shared_metadata"],
                    member_count=member_count,
                    total_size_bytes=coll_meta["total_size"]
                    if coll_meta["total_size"] > 0
                    else None,
                )
                ...
                if validated.include_stats:
                    stats = await self._calculate_collection_stats(
                        str(record.metadata.get("uuid")), include_subcollections=False
                    )
                    collections.append(
                        {"collection": coll_info.model_dump(), "statistics": stats}
                    )
                else:
        ...
        if validated.sort_by == "name":
            collections.sort(
                key=lambda x: x.get("name", x.get("collection", {}).get("name", ""))
            ...
            collections.sort(
                key=lambda x: x.get(
                    "created_at", x.get("collection", {}).get("created_at", "")
                )
                if isinstance(x, dict)
                else x.created_at,
                reverse=True,
            ...
            collections.sort(
                key=lambda x: x.get(
                    "member_count", x.get("collection", {}).get("member_count", 0)
                )
                if isinstance(x, dict)
                else x.member_count,
                reverse=True,
    ...
    async def move_documents(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        if validated.target_collection:
            ...
            if not target_header:
                raise ValueError(
                    f"Target collection not found: {validated.target_collection}"
                )
        ...
        for doc_id in validated.document_ids:
            try:
                ...
                if validated.source_collection:
                    self._remove_document_from_collection(
                        doc_id, validated.source_collection
                    )
                ...
                failed_moves.append({"document_id": doc_id, "error": str(e)})
    ...
    async def get_collection_stats(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        if validated.include_member_details:
            members = self._get_collection_members(
                validated.collection_id, include_content=False
            )
    ...
    def _get_collection_metadata(self, record: FrameRecord) -> dict[str, Any]:
        ...
        return {
            ...
            "shared_metadata": {
                k[7:]: v for k, v in custom.items() if k.startswith("shared_")
            },
        }

    def _set_collection_metadata(
        self, record: FrameRecord, coll_meta: dict[str, Any]
    ) -> None:
        ...
        custom["collection_updated_at"] = coll_meta.get(
    ...
    def _get_collection_header(self, collection_id: str) -> FrameRecord | None:
        ...
        try:
            ...
            filter_str = (
                f"record_type = 'collection_header' AND uuid = '{collection_id}'"
            )
            columns = [
                col
                for col in self.dataset._dataset.schema.names
    ...
    def _add_document_to_collection(
        self, doc_id: str, collection_id: str, header_uuid: str
    ) -> None:
    ...
    def _remove_document_from_collection(self, doc_id: str, collection_id: str) -> None:
        ...
        doc.metadata["relationships"] = [
            ...
            if not (
                rel.get("type") == "reference"
                and collection_id in str(rel.get("id", ""))
            )
        ]
    ...
    def _get_collection_members(
        self, collection_id: str, include_content: bool = True
    ) -> list[dict[str, Any]]:
        ...
        for batch in scanner.to_batches():
            for i in range(len(batch)):
                ...
                for rel in relationships:
                    if rel.get("type") == "reference" and collection_id in str(
                        rel.get("id", "")
                    ):
                        member_info = {
                ...
                subcollections.append(
                    {
                        "collection_id": str(record.metadata.get("uuid")),
                        "name": record.metadata.get("title", "Unnamed"),
                        "member_count": self._get_collection_metadata(record)[
                            "member_count"
                        ],
                    }
                )
    ...
    async def _calculate_collection_stats(
        self, collection_id: str, include_subcollections: bool = True
    ) -> dict[str, Any]:
        ...
        if include_subcollections:
            ...
            for subcoll in subcollections:
                sub_stats = await self._calculate_collection_stats(
                    subcoll["collection_id"], include_subcollections=True
                )
