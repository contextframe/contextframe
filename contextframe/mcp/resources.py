        ...
        try:
            ...
            for record in sample:
                ...
                stats["collections"][collection] = (
                    stats["collections"].get(collection, 0) + 1
                )

                # Count by record type
                record_type = record.metadata.get("record_type", "document")
                stats["record_types"][record_type] = (
                    stats["record_types"].get(record_type, 0) + 1
                )
            ...
            if stats["document_count"] > 0:
                ...
                stats["embedding_coverage"] = (
                    f"{(stats['has_embeddings'] / stats['document_count']) * 100:.1f}%"
                )
