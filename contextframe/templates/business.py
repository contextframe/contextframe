...
class BusinessTemplate(ContextTemplate):
    ...
    # Document patterns
    MEETING_PATTERNS = ["*meeting*", "*minutes*", "*standup*", "*retro*", "*retrospective*"]
    ...
    def _scan_meetings_directory(self, base_path: Path, mappings: list[FileMapping], seen_paths: set):
                ...
                for file_path in meetings_dir.rglob("*"):
                    if file_path.is_file() and file_path not in seen_paths:
                        if file_path.suffix in self.DOC_EXTENSIONS:
                            ...
                            date_match = re.search(r'(\d{4})[-_/](\d{1,2})[-_/](\d{1,2})', str(file_path))
                            if date_match:
                                year, month, day = date_match.groups()
                                mapping.custom_metadata["meeting_date"] = f"{year}-{month:0>2}-{day:0>2}"
    ...
    def _scan_decisions_directory(self, base_path: Path, mappings: list[FileMapping], seen_paths: set):
                ...
                for file_path in decisions_dir.rglob("*"):
                    if file_path.is_file() and file_path not in seen_paths:
                        if file_path.suffix in self.DOC_EXTENSIONS:
                            ...
                            if num_match:
                                mapping.custom_metadata["decision_number"] = num_match.group(1)
                                
                            mapping.collection = "decisions"
                            mappings.append(mapping)
                            seen_paths.add(file_path)
                            
    def _scan_reports_directory(self, base_path: Path, mappings: list[FileMapping], seen_paths: set):
    ...
    def _scan_projects_directory(self, base_path: Path, mappings: list[FileMapping], seen_paths: set):
                ...
                for project_dir in projects_dir.iterdir():
                    if project_dir.is_dir():
                        ...
                        for file_path in project_dir.rglob("*"):
                            if file_path.is_file() and file_path not in seen_paths:
                                if file_path.suffix in self.DOC_EXTENSIONS:
                                    mapping = self._categorize_project_document(file_path, project_name)
    ...
    def _categorize_document(self, file_path: Path) -> FileMapping:
        ...
        if any(pattern.replace("*", "") in name_lower for pattern in self.MEETING_PATTERNS):
            return self._create_meeting_mapping(file_path)
            
        # Check decision patterns
        if any(pattern.replace("*", "") in name_lower for pattern in self.DECISION_PATTERNS):
            return self._create_decision_mapping(file_path)
            
        # Check report patterns
        if any(pattern.replace("*", "") in name_lower for pattern in self.REPORT_PATTERNS):
            return self._create_report_mapping(file_path)
            
        # Check project patterns
        if any(pattern.replace("*", "") in name_lower for pattern in self.PROJECT_PATTERNS):
    ...
    def _create_project_mapping(self, file_path: Path, project_name: str) -> FileMapping:
    ...
    def _categorize_project_document(self, file_path: Path, project_name: str) -> FileMapping:
    ...
    def define_collections(self, file_mappings: list[FileMapping]) -> list[CollectionDefinition]:
        ...
        collection_defs = [
            ("meetings", "Meeting Notes", "Team meetings and discussions", ["meetings", "collaboration"]),
            ("decisions", "Decisions & Proposals", "Strategic decisions and proposals", ["decisions", "strategy"]),
            ("reports", "Reports & Analyses", "Business reports and data analysis", ["reports", "insights"]),
            ("projects", "Project Documentation", "Project plans and documentation", ["projects", "execution"])
        ]
