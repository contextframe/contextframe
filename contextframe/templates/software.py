...
class SoftwareProjectTemplate(ContextTemplate):
    ...
    def scan(self, source_path: str | Path) -> list[FileMapping]:
        ...
        for dir_name in self.SOURCE_DIRS:
            ...
            if src_dir.exists() and src_dir.is_dir():
                mappings.extend(
                    self._scan_code_directory(src_dir, "source", seen_paths)
                )
            ...
            if config_path.exists() and config_path not in seen_paths:
                mappings.append(
                    FileMapping(
                        ...
                        custom_metadata={
                            "config_type": self._get_config_type(config_file)
                        },
                    )
                )
    ...
    def _scan_code_directory(
        self, directory: Path, category: str, seen_paths: set
    ) -> list[FileMapping]:
        ...
        for file_path in directory.rglob("*"):
            ...
            if file_path.suffix in self.CODE_EXTENSIONS:
                ...
                is_test = category == "test" or any(
                    part in file_path.name.lower() for part in ["test", "spec"]
                )
    ...
    def _scan_doc_directory(
        self, directory: Path, seen_paths: set
    ) -> list[FileMapping]:
