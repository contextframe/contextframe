...
class ContextTemplate(abc.ABC):
        ...
        try:
            ...
            if dry_run:
                result.warnings.append(
                    f"DRY RUN: Would import {len(file_mappings)} files"
                )
                return result
            ...
            for coll_def in collections:
                try:
                    ...
                    result.errors.append(
                        f"Failed to create collection {coll_def.name}: {e}"
                    )
            ...
            for mapping in file_mappings:
                ...
                try:
                    ...
                    extraction = extract_from_file(
                        str(mapping.path), **mapping.extract_config
                    )

                    if extraction.error:
                        result.warnings.append(
                            f"Extraction warning for {mapping.path}: {extraction.error}"
                        )
                    ...
                    frame = FrameRecord(
                        text_content=extraction.content, metadata=metadata
                    )
    ...
    def _create_collection(
        self, definition: CollectionDefinition, dataset: FrameDataset
    ) -> FrameRecord:
        ...
        return FrameRecord(text_content=definition.description, metadata=metadata)
