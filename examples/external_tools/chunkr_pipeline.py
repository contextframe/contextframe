...
def extract_with_chunkr(
    ...
    try:
        ...
        raise ImportError(
            "chunkr-ai package is required. "
            "Install with: pip install chunkr-ai"
        )
    ...
    for segment in segments:
        ...
        metadata["segment_types"][seg_type] = metadata["segment_types"].get(seg_type, 0) + 1
...
def process_multimodal_document(
    ...
    logger.info(f"Found: {len(text_chunks)} text, {len(table_chunks)} tables, {len(image_chunks)} images")
